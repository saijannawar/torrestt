...
import { UploadCloud, DollarSign, FileText, Image as ImageIcon, ShieldAlert, Code, CheckCircle, ExternalLink, Loader2 } from 'lucide-react';
...
const AddProduct = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    techStack: '',
    livePreviewUrl: '',
    documentationUrl: '', // Maps to description or separate field if you added one
    subCategoryID: ''
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // A. Upload Thumbnail to S3
      const thumbKey = `thumbnails/${Date.now()}-${thumbnailFile.name}`;
      await uploadData({
        key: thumbKey,
        data: thumbnailFile,
        options: { accessLevel: 'guest' } // Public read for images
      }).result;

      // B. Upload Zip to S3
      const zipKey = `products/${Date.now()}-${zipFile.name}`;
      await uploadData({
        key: zipKey,
        data: zipFile,
        options: { accessLevel: 'guest' } // Protected download
      }).result;
      ...
      await client.graphql({
        ...
        variables: {
          input: {
            name: formData.name,
            description: formData.description + `\n\nDoc Link: ${formData.documentationUrl}`,
      ...
      alert("Error uploading product. Check console details.");
    } finally {
                    ...
                    <div className="space-y-5">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                               ...
                               <select name="subCategoryID" onChange={handleChange} required className="w-full px-4 py-3 border border-gray-300 rounded-lg bg-white focus:outline-none">
                            ...
                            <div>
                               ...
                               <input name="techStack" onChange={handleChange} required type="text" placeholder="e.g. React 18.2 + Tailwind" className="w-full px-4 py-3 border border-gray-300 rounded-lg" />
                            </div>
                        </div>
                    ...
                    <div className="grid grid-cols-1 gap-4">
                        <div>
                            ...
                            <div className="flex">
                                ...
                                <input name="livePreviewUrl" onChange={handleChange} type="url" placeholder="https://demo.torrestt.com" className="w-full px-4 py-2 border border-indigo-200 rounded-r-lg focus:outline-none" />
                            </div>
                        </div>
                        <div>
                            ...
                            <input name="documentationUrl" onChange={handleChange} required type="url" placeholder="https://docs.your-product.com" className="w-full px-4 py-2 border border-indigo-200 rounded-lg focus:outline-none" />
                        </div>
                    </div>
                </div>
            ...
            <div className="space-y-6">
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                    ...
                    <div className="relative">
                        ...
                        <input name="price" onChange={handleChange} required type="number" placeholder="499" className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-lg font-bold text-lg focus:ring-2 focus:ring-green-500 outline-none" />
                    </div>
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                    ...
                    <label className="flex items-start gap-3 cursor-pointer group mb-4">
                        <div className="mt-1">
                            <input required type="checkbox" className="w-4 h-4 rounded border-gray-300 text-black focus:ring-black" />
                        </div>
